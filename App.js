...
const App = () => {
  ...
  const [matAlumno, materiaalumno] = useState('');
  const [cartnetalumno, setcarnet] = useState('');
  ...
  const agregarCliente = () => {
    ...
    const nuevoCliente = { id: clientes.length + 1, nombre: nombre, fechaReserva: fechaReserva, matAlumno: matAlumno, cartnetalumno:cartnetalumno,  };
  ...
  renderItem={({ item }) => (
    ...
    >
      ...
      <Text style={styles.setcarnet}>{item.cartnetalumno}</Text>
